...
However, despite the plethora of techniques available today, there are four primary paradigms of deep learning models that most new methods can be grouped under:
...
The perceptron is recognized as the very first neural network. These fully connected feedforward neural networks are often referred to as Linear or Dense Layers in contemporary deep learning frameworks such as PyTorch and TensorFlow. This class of neural network models excels as universal function approximators. With adequate data, they are capable of learning intricate relationships between inputs and outputs within high-dimensional spaces. They characterize the relationship between an input and an output using a function parameterized by specific weights and biases. In such a model, every node in one layer is connected to all nodes in the subsequent layer. Figure 1 illustrates a straightforward model that utilizes relative humidity and temperature to determine whether it will rain or not.
